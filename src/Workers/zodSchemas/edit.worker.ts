...
const WorkerEditSchema = z.object({
  name: z.string().trim().min(3, 'Name to short, 3 chars minimum').max(50, 'Name too long, 50 chars maximum').optional(),
  secondName: z.string().trim().min(3, 'Second Name to short, 3 chars minimum').max(50, 'Second Name too long, 50 chars maximum').optional(),
  surname: z.string().trim().min(3, 'Surname to short, 3 chars minimum').max(50, 'Surname too long, 50 chars maximum').optional(),
  author: z.number().optional(),
  sex: z.enum(['Salmon', 'Tuna', 'Trout']).optional(),
  ...
  placeOfBirth: z.string().trim().max(50, 'Place of birth too long, 100 chars maximum').optional(),
...
export type WorkerEditSchemaType = z.infer<typeof WorkerEditSchema>;

export default WorkerEditSchema;
