...
const CompanyCreateSchema = z.object({
  name: z.string({ required_error: 'Name is required' }).trim().min(3, 'Email to short, 3 chars minimum').max(50, 'Email to long, 50 chars maximum'),
  nip: z.string({ required_error: 'NIP is required' }).min(10, 'NIP too short, 10 chars minimum').max(10, 'NIP too long, 10 chars maximum').trim(),
  regon: z.string().min(9, 'REGON too short, 9 chars minimum').max(14, 'REGON too long, 14 chars minimum').trim().optional(),
  pkd: z.array(z.string().max(110, 'PKD is too long').trim()).optional(),
